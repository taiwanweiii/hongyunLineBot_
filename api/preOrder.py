    ...
    if apiKey and apiKey == 'mctech':
        ...
        if request.method == 'GET' and page == 'sendMessage':
            ...
            if (orderData):
                ...
                customersDataList = posMemberlDb.dynamicTableSearch({
                    'id':
                    target,
                })
                ...
                if not isinstance(contentList, dict):
                    try:
                        ...
                        productsNamesList = [
                            item['name'] for item in productsDataList
                        ]
                        countList = [
                            item['count'] for item in productsDataList
                        ]
                        productPrice = [
                            item['price'] for item in productsDataList
                        ]
                        countxProductPrice = [
                            x * y for x, y in zip(countList, productPrice)
                        ]
                        productRemixPrice = {
                            name: price
                            for name, price in zip(productsNamesList,
                                                   countxProductPrice)
                        }
                        sorted_productRemixPrice = {
                            k: v
                            for k, v in sorted(productRemixPrice.items(),
                ...
                match titleType:
                    case "charge" if not ErrorMessage:
                        ...
                        templateMoney = copy.deepcopy(
                            template['body']['contents'][2])

                        template['body']['contents'][0]['contents'] = []
                        template['body']['contents'][2]['contents'][1][
                            'text'] = f'{allPrice}'
                        ...
                        for name in (sorted_productRemixPrice):
                            if sorted_productRemixPrice[name] >= 0:
                                ...
                                templateAdd['contents'][1][
                                    'text'] = f'{sorted_productRemixPrice[name]}$'
                                template['body']['contents'][0][
                                    'contents'].append(
                            ...
                            if sorted_productRemixPrice[name] < 0:
                                ...
                                template['body']['contents'][0][
                                    'contents'].append(
                                        copy.deepcopy(templateAdd))
                        line.pushdoubleMessageTextReplyFlex(
                            memberData['userId'], '若有疑問請致電告知\n☎️:0919102803',
                            template, '簽帳單')
                        ...
                        for index, item in enumerate(countxProductPrice):
                            if item >= 1:
                                productNameList.append(
                                    productsNamesList[index])
                        line.pushMessage(
                            memberData['userId'],
                            f'您預定的 {productNameList} 商品已到貨，請記得來店取貨，謝謝😆。')
                    case _:
                        return ErrorMessage
        ...
        elif request.method == 'POST' and page == 'preOrderSendMessage' and all(
                key in value for key in requiredKeys):
